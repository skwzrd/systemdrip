...
def fetch_persistent_metrics():
    ...
    query = "SELECT *, datetime(timestamp, ?) as timestamp_local FROM systemdrip WHERE ROWID % ? = 0 and timestamp > DATE('now', ?);"
    ...
    df = pd.read_sql_query(query, conn, params=[utc_offset_hours, plot_nth_points, plot_n_days])
    return df
